...
SUB     ADD R4, R4, x1      ; Add 1 to R4
        ...
        RET                 ; Return from subroutine
                            ...
                            ; Receive two digit number and save it to R0
...
LEAVE                       ; The LEAVE function begins
    ...
    ADD R1, R1, x5          ; Add 5 to R1
                            ; R1 contains number of digits we have received
...
errorToManyDigits           ; The errorToManyDigits function begins
                            ; Loads the address to ERROR-MSG-TO-MANY-DIGITS into RO
    LEA R0, ERROR-MSG-TO-MANY-DIGITS
    PUTS                    ; Outputs the message to the console
    JSR errorHandling       ; Call errorHandling function
    HALT                    ; Halt the program
    
errorIllegalInput           ; The errorIllegalInput function begins
                            ; Loads the address to ERROR-MSG-ILLEGAL-INPUT into RO
    LEA R0, ERROR-MSG-ILLEGAL-INPUT
    PUTS                    ; Outputs the message to the console
    JSR errorHandling       ; Call errorHandling function
    HALT                    ; Halt the program
    
errorHandling               ; The errorHandling function begins
    LD R0, RESTART          ; Load the address of RESTART and store in R0
    JMP R0                  ; Jump to R0

                            ; Checks that R0 is digit 0 to 9
RESTART .FILL LOOP
checkNumber                 ; The checkNumber function begins
    ST R1, REG1SAVE         ; Store register R1
    AND R1, R1, x0          ; Clear R1
    
	ADD R1, R0, x-9         ; Subtract 9 from R0 and store in R1
	BRp ERROR               ; Branch to ERROR if positive (To big number, n > 9)
	
	NOT R0, R0              ; If we reach this point, the number is not too big,
    NOT R0, R0              ; so we check if it is too small. To do this, we perform
    	                    ; two's complement on R0 (inverting all the bits and then
    	                    ; adding 1). If the result is negative (the sign bit is 1),
...
REG2SAVE        .BLKW 1     ; Reserve memory for REG2SAVE
REG3SAVE        .BLKW 1     ; Reserve memory for REG3SAVE
